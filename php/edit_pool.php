    ...
    <script>
        ...
        function remove_category(category_div_id, category_id, multiple_choice){ 
            if (category_div_id === undefined){ //if category_div_id is undefined, it means that we are removing a new category that hasn't yet been written to DB
                ...
                var pool_mult_choice = <?php echo $pool_fetch_result['Multiple Choice?']; ?>; //we get the multiple choice status of the pool so that we can tell the add_category function in the below replaceWith output that the pool is multiple choice (we need this so that the category space will refresh properly for multiple choice pools when this remove_category function is called)
                $("#"+div_id).replaceWith('<div id="new_category_goes_here"><input type="button" onclick="add_category('+pool_mult_choice+')" value="Add new category"></div>'); //remove new category div and replace it with the original add_category button and its enclosing new_Category_goes_here div:
            }
...
<?php
    if(isset($pool_fetch_result['Template ID'])){ //if this is a template, we make the "Edit info" tab the active tab when the user arrives on the page:
        $edit_info_active_class = "active";
        $edit_categories_active_class = "";
    }
    else{ //if this is NOT a template, we make the "edit categories tab" the active tab when the user arrives at the page
        $edit_info_active_class = "";
        $edit_categories_active_class = "active";   
    }
?>

            <li class="<?php echo $edit_info_active_class; ?>"><a href="#pool_info" data-toggle="tab">Edit Pool Info</a></li>
<?php
    if(!isset($pool_fetch_result['Template ID'])){ //only display the edit categories tab if this is NOT a template
?>
            <li class="<?php echo $edit_categories_active_class; ?>"><a href="#pool_categories" data-toggle="tab">Edit Categories</a></li>
<?php
    }
?>
        ...
        <div id="pool_tab_content" class="tab-content">
            ...
            <!--EDIT POOL INFO DIV -->
            <div class="tab-pane fade in <?php echo $edit_info_active_class; ?>" id="pool_info">
...
?>
            ...
            <div class="tab-pane fade in <?php echo $edit_categories_active_class; ?>" id="pool_categories">
